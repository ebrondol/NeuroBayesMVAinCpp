                            ...
                            bool HbookMode)
{
  ...
  TH1F* targetHisto1 = loadHisto(f,1111,"",HbookMode);
  bool is_splot = targetHisto1!=NULL;
  targetHisto->Draw();
  ...
  const char* title = is_splot ? "Splot weight for signal (target column)"
    : "distribution of target";
  TPaveTextNB *pt1 = new TPaveTextNB(0.33,0.89144,0.67,0.995, title);
...
void analysis( string FileName = "teacherHistos.root",
               const char* PSFileName = "analysis.ps", 
               int sort=0,
               const char* correlSigniFile = "correl_signi.txt") 
{
  ...
  TString working_dir = gSystem->WorkingDirectory();
  TString base_name = gSystem->BaseName(PSFileName);
  TString dir_name = gSystem->DirName(PSFileName);
  ...
  //
  if(outputNodes>1 || loadHisto(f,1000,"",HbookMode) !=NULL) {
    printf("Plotting target distribution\n"); fflush(stdout);
    canvas->Clear();
